...
func TestFindByNameAndVersion(t *testing.T) {
    ...
    repo := repositories.NewConfigurationRepository(c, tracer)

    g.Describe("FindByNameAndVersion", func() {
        g.AfterEach(func() {
            ctx := context.Background()
            _ = repo.DeleteByNameAndVersion(ctx, "golang-test-1", "1.0.0")
            _ = repo.DeleteByNameAndVersion(ctx, "golang-test-1", "1.1.0")
        })

        g.It("should return a configuration", func() {
            ctx := context.Background()
            _, err := repo.Create(ctx, &models.Configuration{Name: "golang-test-1", Version: "1.0.0", Parameters: map[string]string{"db_url": "localhost:1234/db"}})
            if err != nil {
                g.Errorf("Expected no error, got %s", err.Error())
            }

            config, err := repo.FindByNameAndVersion(ctx, "golang-test-1", "1.0.0")
            if err != nil {
                g.Errorf("Expected no error, got %s", err.Error())
            }
        ...
        g.It("should search by name and version", func() {
            ...
            _, err = repo.Create(ctx, &models.Configuration{Name: "golang-test-1", Version: "1.1.0", Parameters: map[string]string{"db_url": "localhost:1234/db"}})
            if err != nil {
                g.Errorf("Expected no error, got %s", err.Error())
            }

            config1, err := repo.FindByNameAndVersion(ctx, "golang-test-1", "1.1.0")
            if err != nil {
                g.Errorf("Expected no error, got %s", err.Error())
            }

            config2, err := repo.FindByNameAndVersion(ctx, "golang-test-1", "1.0.0")
            if err != nil {
                g.Errorf("Expected no error, got %s", err.Error())
            }
        ...
        g.It("should return error if configuration not found", func() {
            ctx := context.Background()
            _, err := repo.FindByNameAndVersion(ctx, "golang-test-1", "1.0.0")
            if err == nil {
...
func TestCreate(t *testing.T) {
    ...
    g.Describe("Create", func() {
        ...
        g.It("should create a configuration", func() {
            ctx := context.Background()
            config, err := repo.Create(ctx, &models.Configuration{Name: "golang-test-1", Version: "1.0.0", Parameters: map[string]string{"db_url": "localhost:1234/db"}})
            if err != nil {
                g.Errorf("Expected no error, got %s", err.Error())
            }
        ...
        g.It("should return error if configuration already exists", func() {
            ctx := context.Background()
            repo.Create(ctx, &models.Configuration{Name: "golang-test-1", Version: "1.0.0", Parameters: map[string]string{"db_url": "localhost:1234/db"}})
            _, err := repo.Create(ctx, &models.Configuration{Name: "golang-test-1", Version: "1.0.0", Parameters: map[string]string{"db_url": "localhost:1234/db"}})
            if err == nil {
                g.Errorf("Expected error, got nil")
            }
